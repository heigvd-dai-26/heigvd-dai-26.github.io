...
`Reader` and `Writer` work with characters, not bytes. `FileReader` and
`FileWriter` read and write text files, and take a **charset**: it tells
Java how to turn bytes into characters, and back.
...
Example [`04-reading-and-writing-text-data`]({{< var examples-url >}}/tree/main/04-java-io/04-reading-and-writing-text-data)
copies its own source file character by character this way.

What is a charset, and what happens if it is wrong? The rest of this
section answers both questions, starting with the second one.
...
Behind `FileReader` there is a chain of streams. The general form builds
it explicitly, and adds a buffer:
...
Example [`05-reading-and-writing-text-data-with-buffers`]({{< var examples-url >}}/tree/main/04-java-io/05-reading-and-writing-text-data-with-buffers)
does the same copy as example 04, with `BufferedReader` and
`BufferedWriter`.
...
binary ones (see [the diagram](#the-reader-chain)).
