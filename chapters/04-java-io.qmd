...
it inside its strings (see below).

A code point above the BMP is stored as a **surrogate pair**: two 16-bit
units taken from a range that Unicode reserves for this purpose
(`U+D800` to `U+DFFF`, where no character is ever assigned). `😂`
(`U+1F602`) becomes the pair `0xD83D 0xDE02`. Neither half is a
character on its own. It is **not compatible with ASCII**: `A`
...
- A `char` is 16 bits: one UTF-16 *unit*, not one character. A code
  point above the BMP (an emoji) needs **two** `char`s, its surrogate
  pair. `"I😂DAI".length()`
