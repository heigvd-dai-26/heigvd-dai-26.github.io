...
Example: `01-reading-and-writing-binary-data`.
...
Example: `02-reading-and-writing-binary-data-with-buffers`.
...
Example: `10-reading-and-writing-blocks`.
...
the start ([exercise 6](#sec-ex6)). Example: `09-byte-order-mark`.
...
- A `char` is 16 bits: one UTF-16 *unit*, not one character. A code
  point above the BMP (an emoji) needs **two** `char`s. `"I😂DAI".length()`
  returns 6, not 5. Use `codePoints()` when you need real characters
  (example `08-code-points`, [exercise 4](#sec-ex4)).
...
Example: `03-character-encodings` encodes "student" in five languages
with five charsets.
...
Examples: `04-reading-and-writing-text-data` (`FileReader` and
`FileWriter`), `05-reading-and-writing-text-data-with-buffers`.
...
Example: `06-end-of-line-characters` (it also uses the full
`InputStreamReader` chain).
...
Example: `07-dealing-with-errors` runs the three versions.
...
Work in the `04-java-io` directory of the
...
2. `utf8.txt` is 5 bytes larger: the five accented letters take 2 bytes
   in UTF-8 instead of 1.
